...
// used: viewmatrix
#include "../core/sdk.h"
#include "../font_sf_pro_display_medium.h"
...
bool ImGui::HotKey(const char* szLabel, unsigned int* pValue)
{
	...
	// Calculate a shorter height for the item
	const float buttonHeight = vecLabelSize.y + style.FramePadding.y * 0.5f;

	// Create the full rect that encompasses label area
	const ImRect rectTotal(
		pWindow->DC.CursorPos,
		pWindow->DC.CursorPos + ImVec2(flWidth, buttonHeight)
	);

	// Account for the item in the layout
	ItemSize(rectTotal, style.FramePadding.y * 0.5f);
	if (!ItemAdd(rectTotal, nIndex, &rectTotal))
		return false;

	// Prepare keybind text
	char szBuffer[64] = {};
	...
	// Calculate keybind button width
	const float keybindWidth = vecBufferSize.x + style.FramePadding.x * 2.0f;

	// Create the keybind button rect on the right side
	const ImRect rectKeybind(
		rectTotal.Max.x - keybindWidth,
		rectTotal.Min.y,
		rectTotal.Max.x,
		rectTotal.Max.y
	);
	...
	const bool bHovered = ItemHoverable(rectKeybind, nIndex, ImGuiItemFlags_None);
	if (bHovered)
	{
		SetHoveredID(nIndex);
	...
	const float textOffsetY = (rectKeybind.GetHeight() - vecBufferSize.y) * 0.5f;

	// Render keybind button (right side)
	RenderFrame(
		rectKeybind.Min,
		rectKeybind.Max,
		GetColorU32((bHovered || bClicked || bDoubleClicked) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg),
		true,
		style.FrameRounding
	);

	// Center the text in the keybind button
	pWindow->DrawList->AddText(
		ImVec2(
			rectKeybind.Min.x + (rectKeybind.GetWidth() - vecBufferSize.x) * 0.5f,
			rectKeybind.Min.y + textOffsetY
		),
		GetColorU32(g.ActiveId == nIndex ? ImGuiCol_Text : ImGuiCol_TextDisabled),
		szBuffer
	);

	// Render label (left side) - align with the reduced height button
	if (vecLabelSize.x > 0.f)
		RenderText(ImVec2(rectTotal.Min.x, rectTotal.Min.y + textOffsetY), szLabel);
...
#pragma endregion
	...
	ImFontConfig main_cfg; main_cfg.PixelSnapH = true;
	main_cfg.GlyphOffset = ImVec2(0, 0);
	main_cfg.FontBuilderFlags |= ImGuiFreeTypeBuilderFlags::ImGuiFreeTypeBuilderFlags_MonoHinting;
	main_cfg.SizePixels = 16.0f;
	for (int i = 0; i < CS_ARRAYSIZE(FONT::pMenu); i++)
	{
		const float flFontSize = 12.f * CalculateDPI(i);
		FONT::pMenu[i] = io.Fonts->AddFontFromMemoryCompressedTTF(font_sf_pro_display_medium_compressed_data, font_sf_pro_display_medium_compressed_size, main_cfg.SizePixels, &main_cfg, io.Fonts->GetGlyphRangesDefault());
		//FONT::pMenu[i] = io.Fonts->AddFontFromMemory(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), flFontSize, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
	...
	FONT::pVisual = io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Tahoma.ttf"), 11.f, &imTahomaConfig, io.Fonts->GetGlyphRangesCyrillic());
