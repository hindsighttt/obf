...
void c_skins::dump_items() {
    ...
    for (const auto& it : vec_items) {
        ...
        if (!dumped_item.m_dumped_skins.empty() && is_weapon)
            std::sort(dumped_item.m_dumped_skins.begin(), dumped_item.m_dumped_skins.end(),
                [](const c_dumped_skins& a, const c_dumped_skins& b) { return a.m_rarity > b.m_rarity; });
...
void c_skins::glove_changer(c_base_view_model* view_model) {
    ...
    if ((model_skin != last_skin && model_skin > 0) || local_player->m_spawn_time_index() != last_spawn_time) {
        static auto create_new_paint_kit = reinterpret_cast<c_paint_kit * (__fastcall*)(c_econ_item_view*)>(
            g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 89 5C 24 10 56 48 83 EC 20 48 8B 01 FF 50 10 48 8B 1D ? ? ? ?")
        );
