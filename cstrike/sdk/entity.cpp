...
bool CEconItemDefinition::IsKnife(bool excludeDefault) {
	static constexpr auto CSGO_Type_Knife =
		FNV1A::hash_32_fnv1a_const("#CSGO_Type_Knife");

	if (FNV1A::hash_32_fnv1a_const(m_pszItemTypeName) != CSGO_Type_Knife) return false;
	return excludeDefault ? m_nDefIndex >= 500 : true;
}

bool CEconItemDefinition::IsGlove(bool excludeDefault) {
	static constexpr auto Type_Hands = FNV1A::hash_32_fnv1a_const("#Type_Hands");

	if (FNV1A::hash_32_fnv1a_const(m_pszItemTypeName) != Type_Hands) return false;
...
{
	if (!H::hkSetModel.IsHooked()) return;
	H::SetModel(this, name);
}

float C_CSWeaponBase::GetInaccuracy()
{
	static auto pGetWeaponInaccuracy = reinterpret_cast<decltype(&H::GetWeaponInaccuracy)>(
		MEM::FindPattern(CLIENT_DLL, "48 89 5C 24 10 55 56 57 48 81 EC B0"));
	float x, y;
	return pGetWeaponInaccuracy(this, nullptr, nullptr);
}
