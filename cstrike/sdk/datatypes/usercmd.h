...
public:
	...
	CSubtickMoveStep* add_subtick_move()
	{
		...
		CSubtickMoveStep* subtick = fn_create_new_subtick_move_step(subtickMovesField.pArena);
	...
	{
		return H::CreateSubTickStepMove(arena);
	}
};
...
public:
	...
	void CheckAndSetBits2(std::uint32_t nBits)
	{
...
public:
	...
	void AddSubTickShot()
	{
		for (int i = 0; i < this->csgoUserCmd.inputHistoryField.pRep->nAllocatedSize; i++)
		{
			...
			auto subtick = this->csgoUserCmd.pBaseCmd->create_subtick_step(this->csgoUserCmd.inputHistoryField.pArena);
			subtick->bPressed = true;
			subtick->nButton = IN_ATTACK;
		}
	}
};
