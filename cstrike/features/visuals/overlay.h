...
namespace F::VISUALS::OVERLAY
{
	...
	void AddText(const char* Text, ImVec2 Position, ImColor Color);

	void AddTextFlag(const char* Text, ImVec2 Position, ImColor Color);
	...
	void FOVCircle();
	void ScopeOverlay();
	bool CanWallbang(float& dmg, CCSPlayerController* pLocalController);
	void SpreadCircle(CCSPlayerController* pLocal);
	void PenetrationCrosshair(CCSPlayerController* pLocal);
}
