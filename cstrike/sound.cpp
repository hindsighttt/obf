...
void Waveform::WavHeader_t::ParseWavHeader(unsigned char* wavData) {
...
bool Waveform::AdjustHitsoundVolume(Waveform::WavHeader_t& header, float volume)
{
...
void Waveform::Play(float flVolume)
{
	if (m_should_parse_hitsound) {
		parsed_hitsound = skeet;

		Waveform::WavHeader_t header;
		header.ParseWavHeader(parsed_hitsound);
		AdjustHitsoundVolume(header, flVolume);
